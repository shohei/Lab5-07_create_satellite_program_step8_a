...
HEPTA_COM::HEPTA_COM(PinName tx, PinName rx, int rate) : xbee(tx, rx) { // デジタル出力の実体化
        xbee.baud(rate);
        rcmd = 0;
        cmdflag = 0;
}

void HEPTA_COM::xbee_receive(int *xrcmd, int *xcmdflag)
{
    xbee.attach(this,&HEPTA_COM::commandget,Serial::RxIrq);
    //xbee.printf("ssXbeedata= %d,%d",HeptaXbee::rcmd,HeptaXbee::cmdflag);
    *xrcmd = HEPTA_COM::rcmd;
    *xcmdflag = HEPTA_COM::cmdflag;
}
